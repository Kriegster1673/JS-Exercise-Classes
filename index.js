...
class Person {
  ...
  eat(edible){
    if (this.stomach.length >= 10) {return 'Belly is full'}
...
class Instructor extends Lambdasian {
  constructor(yup){
  super(yup)
  this.specialty = yup.specialty
  this.favLanguage = yup.favLanguage
  this.catchPhrase = yup.catchPhrase
  }
  demo(subject){
...
class Student extends Lambdasian{
   constructor(yup){
     super(yup)
     this.previousBackground = yup.previousBackground
     this.className = yup.className
     this.favSubjects = yup.favSubjects
   }
...
class ProjectManager extends Instructor {
  constructor(yup){
    super(yup)
    this.gradClassName = yup.gradClassName
    this.favInstructor = yup.favInstructor
  }
  standUp(slack){return `${this.name} announces to ${slack}, @channel standy times!`}
